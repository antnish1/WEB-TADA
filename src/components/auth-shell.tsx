...
export function AuthShell() {
  ...
  if (!isSupabaseConfigured || !supabase) return <div className="auth-screen"><div className="auth-card"><h1>Supabase not configured</h1><p>Create <code>.env.local</code> using the supplied project URL and anon key, then restart the development server.</p></div></div>;
  ...
  if (!elevated && !assignedBranch) return <div className="auth-screen"><div className="auth-card"><h1>Branch not assigned</h1><p>Your profile does not have an active branch. Contact the WEB-TADA administrator.</p><button className="auth-submit" onClick={signOut}>Sign out</button></div></div>;
